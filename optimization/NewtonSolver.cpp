...
void NewtonSolver::Init()
{
  ...
  if(origProblem.d != NULL) {
    ...
    if(!TestGradient(barrierProblem->f,x,0.01,1e-3)) {
      LOG4CXX_ERROR(logger,"Barrier gradient is incorrect"<<"\n");
      if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
    }
    if(!TestHessian(barrierProblem->f,x,0.01,1e-3)) {
      LOG4CXX_ERROR(logger,"Barrier hessian is incorrect"<<"\n");
      if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
    }
    */
  }
  ...
  if(!res) {
    LOG4CXX_ERROR(logger,"Warning, couldn't compute the SVD of the constraint Jacobian"<<"\n");
    LOG4CXX_ERROR(logger,"Using zero for the lagrange multipliers..."<<"\n");
    a.setZero();
    if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
  }
  if(verbose >= 2) {
    ...
    LOG4CXX_INFO(logger,"Constraint Jacobian is"<<"\n"<<MatrixPrinter(A)<<"\n");
    LOG4CXX_INFO(logger,"Gradient of objective is "<<VectorPrinter(vtemp)<<"\n");
    LOG4CXX_INFO(logger,"Lambda is "<<VectorPrinter(a)<<"\n");
    LOG4CXX_INFO(logger,"Residual with lambda at start: "<<VectorPrinter(temp)<<"\n");
  }
  */
  a.set(One);
  H.resize(x.n,x.n);
...
ConvergenceResult NewtonSolver::Solve(int& iters)
{
  ...
  for(iters=0;iters<maxIters;iters++) {
    ...
    if(verbose >= 1)
      LOG4CXX_INFO(logger,"Step "<<iters<<", function value "<<problem->f->Eval(x)<<", constraint dist "<<res2.maxAbsElement()<<"\n");
    if(verbose >= 2) {
      LOG4CXX_INFO(logger,"Initial: "<<VectorPrinter(x)<<", "<<VectorPrinter(a)<<"\n");
    ...
    if(d.minAbsElement() <= 1e-5) {
      if(verbose >= 1) LOG4CXX_ERROR(logger,"Hessian uninvertible... LDL diagonal gives "<<d.minAbsElement()<<"\n");
      for(int i=0;i<d.n;i++) {
	if(d(i) < 0) d(i) -= 1e-3;
	else d(i) += 1e-3;
      }
    ...
    if(d.minAbsElement() <= 1e-5) {
      if(verbose >= 1) LOG4CXX_ERROR(logger,"AHA matrix uninvertible... LDL diagonal gives "<<d.minAbsElement()<<"\n");
      for(int i=0;i<d.n;i++) {
	if(d(i) < 0) d(i) -= 1e-3;
	else d(i) += 1e-3;
      }
    ...
    Hinv.mul(vtemp,dx);
    if(verbose >= 2) LOG4CXX_INFO(logger,"Direction: "<<VectorPrinter(dx)<<", "<<VectorPrinter(da)<<"\n");
...
ConvergenceResult NewtonSolver::LineSearch(Real& alpha)
{
  ...
  if(verbose >= 2) LOG4CXX_INFO(logger,"Initial merit="<<fx0<<"\n");
  Real t = alpha;
  Real gnorm = Max(dx.maxAbsElement(),da.maxAbsElement());
  for(int iters=0;;iters++) {
    if(Abs(t) * gnorm < tolx) {
      if(verbose>=1) LOG4CXX_INFO(logger,"NewtonSolver::LineSearch(): Quitting on line search iter "<<iters<<"\n");
      x = x0;
      a = a0;
      alpha = 0;
      return ConvergenceX;
    ...
    problem->PreEval(x);
    fx = Merit(x,a);
    if(verbose >= 2) LOG4CXX_INFO(logger,"Step "<<t<<" merit="<<fx<<"\n");
    if(fx < fx0-1e-4*gnorm*Abs(t)) break;

    t *= 0.5;
  }
  if(verbose >= 2) LOG4CXX_INFO(logger,"Decreased merit function from "<<fx0<<" to "<<fx<<" with step "<<t<<"\n"); 
  alpha = t;
  if(S) S->push_back(x);
