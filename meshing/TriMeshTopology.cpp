...
void TriMeshWithTopology::CalcTriNeighbors()
{
  ...
  for(size_t i=0;i<tris.size();i++) {
	...
	if(triNeighbors[i].c!=-1) {
	  LOG4CXX_ERROR(logger,"TriMeshTopology: mesh has two neighbors on the same edge!"<<"\n");
	  LOG4CXX_ERROR(logger,"Triangle "<<i<<" abuts triangles "<<triNeighbors[i].c<<" and "<<k<<" on edge a-b"<<"\n");
	  //LOG4CXX_ERROR(logger,"Triangle "<<i<<": "<<tris[i]<<"\n");
	  //LOG4CXX_ERROR(logger,"Neighbor 1: "<<tris[triNeighbors[i].c]<<"\n");
	  //LOG4CXX_ERROR(logger,"Neighbor 2: "<<tris[k]<<"\n");
	...
	if(triNeighbors[i].b!=-1) {
	  LOG4CXX_ERROR(logger,"TriMeshTopology: mesh has two neighbors on the same edge!"<<"\n");
	  LOG4CXX_ERROR(logger,"Triangle "<<i<<" abuts triangles "<<triNeighbors[i].b<<" and "<<k<<" on edge a-c"<<"\n");
	  //if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
	}
	triNeighbors[i].b=k;
      }
	...
	if(triNeighbors[i].a!=-1) {
	  LOG4CXX_ERROR(logger,"TriMeshTopology: mesh has two neighbors on the same edge!"<<"\n");
	  LOG4CXX_ERROR(logger,"Triangle "<<i<<" abuts triangles "<<triNeighbors[i].a<<" and "<<k<<" on edge b-c"<<"\n");
	  //if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
	}
	triNeighbors[i].a=k;
      }
...
void TriMeshWithTopology::SplitEdge(int tri,int e,const Vector3& newPt)
{
  ...
  if(!tris[adj].contains(b,ind1)) {
    LOG4CXX_INFO(logger,"Internal inconsistency!"<<"\n");
    abort();
  }
