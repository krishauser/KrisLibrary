...
void MergeVertices(TriMeshWithTopology& mesh,Real tolerance,bool dropDegenerate)
{
  vector<Vector3> newpts;
  vector<int> newmap(mesh.verts.size(),-1);
  if(tolerance==0) {
  ...
  if(dropDegenerate) 
    Assert(mesh.IsValid());
  if(!mesh.vertexNeighbors.empty()) {
    mesh.vertexNeighbors.clear();
    mesh.CalcVertexNeighbors();
  }
  if(!mesh.incidentTris.empty()) {
    mesh.incidentTris.clear();
    mesh.CalcIncidentTris();
  }
  if(!mesh.triNeighbors.empty()) {
    mesh.triNeighbors.clear();
