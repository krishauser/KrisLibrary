...
Real TraceToTheta(Real trace)
{
  ...
  if(s >= One) {
    if(s > One+Epsilon) {
      cerr<<"TraceToTheta(): Warning- trace of matrix is greater than 1"<<endl;
    }
    return Zero;
  }
  else if(s <= -One) {
    if(s < -One-Epsilon) {
      cerr<<"TraceToTheta(): Warning- trace of matrix is less than 1"<<endl;
    }
    return Pi;
  }
  else
...
void DirectionCosines::getMomentJacobian(Matrix& J) const
{
  ...
  if(FuzzyEquals(theta,Pi)) {
    cout<<"Discontinuity: theta is pi"<<endl;
    J.resize(3,9,Zero);
...
void DirectionCosines::getQuaternionJacobian(Matrix& J) const
{
  ...
  if(tr > 1e-2) {
    ...
    cout<<"Close to pi rotations not done yet"<<endl;
    J.setZero();
    getchar();
  }
}
...
void GetMinimalRotation(const Vector3& x,const Vector3& y,Matrix3& R)
{
  ...
  if(FuzzyZero(n)) {
	...
	cerr<<"GetMinimalRotation: Warning: vectors aren't normalized?"<<endl;
	getchar();
      }
...
Real MatrixAbsoluteAngle(const Matrix3& R)
{
  ...
  if(!IsFinite(s)) {
    cerr<<"MatrixAbsoluteAngle(): Warning- trace of matrix is not finite!"<<endl;
    cerr<<R<<endl;
    Abort();
  }
  if(s >= One) {
    if(s > One+Epsilon) {
      cerr<<"MatrixAbsoluteAngle(): Warning- trace of matrix is greater than 3"<<endl;
    }
    return Zero;
  }
  else if(s <= -One) {
    if(s < -One-Epsilon) {
      cerr<<"MatrixAbsoluteAngle(): Warning- trace of matrix is less than -1"<<endl;
    }
    return Pi;
  }
  else 
