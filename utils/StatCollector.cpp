...
{
  if(!InputQuotedString(in,name)) { LOG4CXX_INFO(logger,"Failed to load name"<<"\n"); return false; }
  in>>data.count;
  if(!data.value.Load(in))  { LOG4CXX_INFO(logger,"Failed to load value for "<<name<<"\n"); return false; }

  EatWhitespace(in);
  int c = in.get();
  if (!in) {
	  LOG4CXX_INFO(logger, "Couldnt load open brace for " << name.c_str() << ", end of file\n");
  }
  if (c != '{') { LOG4CXX_INFO(logger, "Couldn't load open brace for " << name.c_str() << ", got " << c); return false; }
  EatWhitespace(in);
  c = in.peek();
  while(c != '}') {
    string childname;
...
bool StatDatabase::Load(istream& in)
{
  ...
  if(LoadRecurse(in,temp,root,0)) {
    if(temp != "StatDatabase") {
      LOG4CXX_ERROR(logger,"StatDatabase::Load(): Warning, root is not set to \"StatDatabase\""<<"\n");
    }
    return true;
  }
  return false;
