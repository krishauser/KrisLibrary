...
void Plane3D::setTransformed(const Plane3D& pin, const Matrix4& xform)
{
  ...
  LOG4CXX_ERROR(logger,"Using slow version of Plane3D::setTransformed, may want to use RigidTransform version"<<"\n");
...
int Plane3D::allIntersections(const Plane3D& p,Line3D& l) const
{
  ...
  if(LinearlyDependent_Robust(normal,p.normal,c,cb,eps)) {
    ...
    if(FuzzyZero(N.determinant())) {
      //LOG4CXX_ERROR(logger,"That's strange, I thought these weren't linearly dependent"<<"\n");
      return 0;
    }
    N.inplaceInverse();
    N.mul(o,u);
    ...
    if(!Fuzzyzero(distance(l.source),Epsilon)) {
      LOG4CXX_ERROR(logger,"Hmm... plane-plane intersection has a weird result: "<<"\n");
      LOG4CXX_ERROR(logger,"distance to line point "<<l.source<<" is "<<distance(l.source)<<"\n");
      LOG4CXX_ERROR(logger,"determinant of normal equations: "<<N.determinant()<<"\n");
      LOG4CXX_INFO(logger,"p1 : normal "<<normal<<" offset "<<offset<<"\n");
      LOG4CXX_INFO(logger,"p2 : normal "<<p.normal<<" offset "<<p.offset<<"\n");
      if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
    }
