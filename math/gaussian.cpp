...
bool Gaussian<T>::setCovariance(const MatrixT& sigma,int verbose)
{
  ...
  for(int i=0;i<D.n;i++) {
    if(D(i) < 0.0) {
      if(verbose >= 1) 
	printf("Gaussian::setCovariance(): Warning, sigma %d = %g is not positive semi-definite\n",i,D(i));
      if(D(i) < -Epsilon) 
	res = false;
      D(i) = 0.0;
    }
...
bool Gaussian<T>::setMaximumLikelihood(const std::vector<VectorT>& examples,const std::vector<Real>& weights,int verbose)
{
  ...
  if(sumw == 0.0) {
    ...
    if(verbose >= 1)
      printf("Gaussian::setMaximumLikelihood: weights sum to zero\n");
    return false;
  }
...
void Gaussian<T>::setJoint(const MyT& g1,const MyT& g2,const MatrixT& corr)
{
  ...
  if(!LBackSubstitute(g1.L,corr,Xt)) {
    printf("Warning: Gaussian::setJoint failed!\n");
    X.setZero();
  }
}
