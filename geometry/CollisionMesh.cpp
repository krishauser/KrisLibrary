...
Real CollisionMeshQuery::PenetrationDepth()
{
  ...
  if(penetration1->maxDepth <= 0 && penetration2->maxDepth <= 0) {
    ...
    if(d > 1e-3) {
      LOG4CXX_ERROR(logger,"PenetrationDepth(): Error, the two objects aren't penetrating?!?!"<<"\n");
      LOG4CXX_INFO(logger,"Distance "<<d<<"\n");
      Abort();
    }
    LOG4CXX_WARN(logger,"PenetrationDepth(): Warning, the approximate computation failed, returning "<<Max(-d,(Real)1e-5)<<"\n");
  ...
  for(map<int,PQP_ClosestPoints >::const_iterator i=allRes.triCp1.begin();i!=allRes.triCp1.end();i++) {
    if(allRes.triDist1.find(i->first)->second == 0) {
      ...
      if(OverlappingTriangleCollision(m1,m2,i->first,allRes.triPartner1.find(i->first)->second,pl1,pl2)) {
        ...
        LOG4CXX_WARN(logger,"Warning, PQP detected overlapping triangles but we find they don't intersect?\n");
        continue;
      }
    }
    else {
...
{
  if(penetration1->maxDepth <= 0 && penetration2->maxDepth <= 0) {
    LOG4CXX_ERROR(logger,"PenetrationDepth_Cached(): Error, the two objects have no interior vertices!"<<"\n");
    Abort();
  }
  if(penetration1->maxDepth <= 0) return penetration2->maxDepth;
  else if(penetration2->maxDepth <= 0) return penetration1->maxDepth;
...
void CollisionMeshQuery::PenetrationPoints(Vector3& p1,Vector3& p2,Vector3& d1) const
{
  ...
  if(penetration1->maxDepth <= 0 && penetration2->maxDepth <= 0) {
    LOG4CXX_WARN(logger,"PenetrationPoints(): Warning, the two objects have no interior vertices!"<<"\n");
    ...
    d1 = p2-p1;
    d1.inplaceNormalize();
    LOG4CXX_INFO(logger,"Returning closest points "<<p1<<", "<<p2<<", dir "<<d1<<"\n");
    //if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
    return;
  }
  else {
...
struct ClosestPointCallback
{
  ...
  void Execute(const PQP_Model& m,const Vector3& p) {
    ...
    while(!q.empty()) {
      ...
      numBBsChecked++;
      LOG4CXX_INFO(logger,"Popped bound with min distance: "<<temp.minDist<<"\n");
      int b=temp.index;
	...
	LOG4CXX_INFO(logger,"Children "<<c1<<", "<<c2<<", distances "<<dbmin1<<", "<<dbmin2<<"\n");
	...
	if(dbmin1 < dmax) {
	  ...
	  q.insert(temp);
	  if(dbmax1 < dmax) {
	    LOG4CXX_INFO(logger,"Max distance set to "<<dbmax1<<" on child 1"<<"\n");
	    dmax = dbmax1;
	    prune = true;
	  }
	}
	if(dbmin2 < dmax) {
	  ...
	  q.insert(temp);
	  if(dbmax2 < dmax) {
	    LOG4CXX_INFO(logger,"Max distance set to "<<dbmax2<<" on child 2"<<"\n");
	    dmax = dbmax2;
	    prune = true;
	  }
	}
    ...
    LOG4CXX_INFO(logger,numTrianglesChecked<<" triangles, "<<numBBsChecked<<" BBs checked"<<"\n");
    */
  }
...
int ClosestPoint(const CollisionMesh& mesh,const Vector3& p,Vector3& cp)
{
  ...
  if(!FuzzyEquals(cb.cp.distance(p),cp.distance(p),1e-3)) {
    LOG4CXX_ERROR(logger,"ClosestPoint error: "<<cb.cp.distance(p)<<" != "<<cp.distance(p)<<"\n");
    LOG4CXX_ERROR(logger,"Triangle "<<cb.closestTri<<" vs "<<regularTri<<"\n");
  }
...
bool Collide(const CollisionMesh& m,const GeometricPrimitive3D& g)
{
  ...
  case GeometricPrimitive3D::Empty:
    return false;
  default:
        LOG4CXX_ERROR(logger,"Collide: Collider for type "<<g.TypeName());
    return false;
  }
}
...
void CollideAll(const CollisionMesh& m,const GeometricPrimitive3D& g,std::vector<int>& tris,int max)
{
  ...
  case GeometricPrimitive3D::Empty:
    return;
  default:
        LOG4CXX_ERROR(logger,"CollideAll: Collider for type "<<g.TypeName());
  }
}
...
void NearbyTriangles(const CollisionMesh& m,const GeometricPrimitive3D& g,Real d,vector<int>& tris,int max)
{
  ...
  case GeometricPrimitive3D::Empty:
    return;
  default:
        LOG4CXX_ERROR(logger,"NearbyTriangles: Collider for type "<<g.TypeName());
  }
}
            ...
            PQP_REAL p[3], PQP_REAL q[3])
{
  ...
  if(d == 0) {  //TriDistance doesn't give the right points
    ...
    if(!res) {
      LOG4CXX_WARN(logger,"Warning: PQP says triangles intersect, but I don't find an intersection\n");
