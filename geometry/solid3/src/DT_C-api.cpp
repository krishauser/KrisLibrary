...
DT_ShapeHandle DT_NewHullFree(DT_ShapeHandle shape1, DT_ShapeHandle shape2) {
    ...
    return (DT_ShapeHandle)new DT_HullFree(*reinterpret_cast<DT_Convex *>(shape1), *reinterpret_cast<DT_Convex *>(shape2));
}

DT_ShapeHandle DT_OldPolytope(const DT_VertexBaseHandle vertexBase, DT_Count count, const DT_Index *indice) {
	return (DT_ShapeHandle)new DT_Polytope(reinterpret_cast<DT_VertexBase *>(vertexBase), count, indice); 
}
...
void DT_RemoveObject(DT_SceneHandle scene, DT_ObjectHandle object) 
{
    assert(scene);
    assert(object);
    reinterpret_cast<DT_Scene *>(scene)->removeObject(*reinterpret_cast<DT_Object *>(object));
}
				    ...
				    DT_Vector3 point1, DT_Vector3 point2) 
{
	...
	MT_Vector3  v(MT_Scalar(0.1), MT_Scalar(0.0), MT_Scalar(0.0)); 
    MT_Point3   p1, p2;
...
void DT_RemoveDefaultResponse(DT_RespTableHandle respTable,
							  DT_ResponseCallback response)
{
      reinterpret_cast<DT_RespTable *>(respTable)->removeDefault(DT_Response(response));
}
