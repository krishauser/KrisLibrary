...
public:
	...
	void get_support(const double *dir, double *out) {
		MT_Vector3 v(dir[0], dir[1], dir[2]);
		DT_Convex *shape = reinterpret_cast<DT_Convex*>(const_cast<DT_Shape*>(&this->m_shape));
		DT_Transform obj(m_xform, *shape);
		MT_Point3 rst = obj.support(v);
		for(int i = 0; i < 3; i++)
			out[i] = rst[i];
	}
	...
	const DT_Shape& get_shape() const {return m_shape;}
