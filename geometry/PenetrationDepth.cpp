...
void ApproximatePenetrationDepth::ComputeInitial(const RigidTransform& f1,const RigidTransform& f2,const int tc1[], const int tc2[],int n)
{
  ...
  for(int i=0;i<n;i++) {
    ...
    T2.c = f2*T2.c;
    if(!T1.intersect(T2)) {
      LOG4CXX_ERROR(logger,"Error, T1 doesn't intersect T2!"<<"\n");
      LOG4CXX_INFO(logger,"i="<<i<<"\n");
      LOG4CXX_INFO(logger,T1.a<<", "<<T1.b<<", "<<T1.c<<"\n");
      LOG4CXX_INFO(logger,T2.a<<", "<<T2.b<<", "<<T2.c<<"\n");
    }
  ...
  */
  ...
  for(int i=0;i<n;i++) {
    ...
    */
    ...
    if(Max(da,db,dc) < Zero || Min(da,db,dc) > Zero) {
      //LOG4CXX_WARN(logger,"Warning: triangle "<<tc1[i]<<" does not intersect "<<tc2[i]<<"\n");
      continue;
    }
...
#if VERBOSE
  if(!collisions.empty())
    LOG4CXX_INFO(logger,collisions.size()<<" edge collisions."<<"\n");
#endif
  ...
  for(Iterator i=collisions.begin();i!=collisions.end();i++) {
	...
	if(j->second.voutside == i->second.vinside) break;
      if(j != range.second) {
	//LOG4CXX_INFO(logger,"Looks like an edge penetration..."<<"\n");
	...
	if(p1.distance(a) > Epsilon) {
	  LOG4CXX_ERROR(logger,"Warning: distance from triangle to inside point is greater than 0! "<<p1.distance(a)<<"\n");
	  if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
	}
	if(p2.distance(b) > Epsilon) {
	  LOG4CXX_ERROR(logger,"Warning: distance from triangle to inside point is greater than 0! "<<p2.distance(b)<<"\n");
	  if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
	}
	...
	//LOG4CXX_INFO(logger,"Hmm... the outside vertex is a fringe vertex, what do we do?"<<"\n");
      }
    }
  ...
  for(Iterator i=collisions.begin();i!=collisions.end();) {
	...
	LOG4CXX_WARN(logger,"PenetrationDepth::ComputeInitial(): Warning, initial point has a negative depth"<<"\n");
	//if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
      }
...
#if VERBOSE
  if(maxDepth != Zero)
    LOG4CXX_INFO(logger,"MaxDepth "<<maxDepth<<", "<<visitedNodes<<" of "<<m1.verts.size()<<"\n");
#endif
}
