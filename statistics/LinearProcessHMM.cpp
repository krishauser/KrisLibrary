...
bool LinearProcessHMM::TrainEM(const vector<vector<Vector> >& examples,Real& tol,int maxIters,int verbose)
{
  ...
  for(int iters=0;iters<maxIters;iters++) {
    ...
    LOG4CXX_INFO(logger,"Log likelihood of data: "<<lltotal);
    if(Abs(lltotal-lltotal_old) < tol) {
      return true;
    }
    if((iters+1) % 10 == 0) {
      LOG4CXX_INFO(logger,"Saving progress to temp.lphmm\n");
      ofstream out("temp.lphmm");
      out<<*this<<endl;
      out.close();
    }

    LOG4CXX_INFO(logger,"Performing E step iteration "<<iters);
    ...
    //M step
    LOG4CXX_INFO(logger,"Performing M step...\n");
    //first maximize discrete prior
    discretePrior.setZero();
    for(size_t i=0;i<examples.size();i++) {
      discretePrior += w[i][0];
      /*
      LOG4CXX_INFO(logger,"Initialization probability "<<i<<": "<<w[i][0]<<"\n");
      LOG4CXX_INFO(logger,"Emission probabilities: ");
      for(int j=0;j<discretePrior.n;j++)
	LOG4CXX_INFO(logger,""<<emissionModels[j].probability(examples[i][0]));
      LOG4CXX_INFO(logger,"\n");
      */
    }
    NormalizeProbability(discretePrior);
    //LOG4CXX_INFO(logger,"Priors: "<<discretePrior<<"\n");
    //now maximize transition matrix
    transitionMatrix = taccum;
    //LOG4CXX_INFO(logger,"Counts: "<<transitionMatrix<<"\n");
	...
	LOG4CXX_ERROR(logger,"Error doing OLS\n");
	...
	//LOG4CXX_ERROR(logger,"Terminated by error doing OLS\n");
	//return false;
      }
      else {
	...
	if(predictionHorizon != 1) {
	  ...
	  LOG4CXX_INFO(logger,"Prediction A"<<"\n");
	  LOG4CXX_INFO(logger,emissionModels[i].A<<"\n");
	  ...
	  for(int p=0;p<emissionModels[i].A.m;p++) {
	    ...
	    if(!IsFinite(scale) || scale < 0.0) {
	      LOG4CXX_INFO(logger,"Sign change "<<scale);
	    }
	    else {
	      scale = Sqrt(scale);
	      for(int q=0;q<emissionModels[i].A.m;q++) {
...
#if PRINT_PREDICT_FIT
	  LOG4CXX_INFO(logger,"one step (1/k'th power) ~="<<"\n");
	  LOG4CXX_INFO(logger,emissionModels[i].A<<"\n");
	  LOG4CXX_INFO(logger,"raised to k'th power ="<<"\n");
	  ...
	  if(!res) {
	    LOG4CXX_INFO(logger,"Couldn't solve for mean\n");
	    emissionModels[i].error.mu /= predictionHorizon;
	  }
	  else {
#if PRINT_PREDICT_FIT
	    LOG4CXX_ERROR(logger,"Original predicted mean"<<emissionModels[i].error.mu<<"\n");
	    LOG4CXX_INFO(logger,"New 1-step mean"<<vtemp<<"\n");
#endif // PRINT_PREDICT_FIT
...
#if PRINT_PREDICT_FIT
	  LOG4CXX_INFO(logger,"Predicted covariance"<<"\n");
	  LOG4CXX_INFO(logger,covdes<<"\n");
#endif
	  //stack rhs
	  ...
	  if(!res) {
	    LOG4CXX_INFO(logger,"Couldn't solve for covariance\n");
	    ...
	    if(!res) {
	      LOG4CXX_ERROR(logger,"Error setting covariance matrix"<<"\n"<<covdes<<"\n");
...
#if PRINT_PREDICT_FIT
	      LOG4CXX_INFO(logger,"Solved 1-step covariance"<<"\n");
	      LOG4CXX_INFO(logger,covdes<<"\n");
#endif // PRINT_PREDICT_FIT
	    }
...
#if PRINT_PREDICT_FIT
	    LOG4CXX_INFO(logger,"Compounded covariance"<<"\n");
	    temp = covdes;
	...
	for(int j=0;j<emissionModels[i].error.L.n;j++)
      ...
      //LOG4CXX_INFO(logger,"Center "<<i<<": "<<emissionModels[i].mu<<"\n");
    }
  }
  return true;
}
...
Real LinearProcessHMM::LogLikelihood(const vector<Vector>& observations) const
{
  ...
  if(pobs0 == 0) {
    LOG4CXX_WARN(logger,"Warning, probability of initial observation is 0\n");
    LOG4CXX_INFO(logger,"Discrete distribution "<<discretePrior<<"\n");
  }
  ll += Log(pobs0);
  for(size_t i=1;i<observations.size();i++) {
    ...
    if(pi == 0) {
      LOG4CXX_WARN(logger,"Warning, probability of transition "<<i);
      LOG4CXX_INFO(logger,"Discrete distribution "<<temp<<"\n");
      LOG4CXX_INFO(logger,"Observations: "<<observations[i-1]<<" "<<observations[i]<<"\n");
      for(int j=0;j<discretePrior.n;j++)
	LOG4CXX_INFO(logger,"Logp["<<j<<"] = "<<emissionModels[j].LogProbability(observations[i-1],observations[i])<<"\n");
    }
    ll += Log(pi);
...
Real LinearProcessHMMRegression::ProbabilityX(const LinearProcessHMMState& s,const Vector& x) const
{
  ...
  if(s.xPrev.empty() && s.x.empty()) {
    ...
    Real p=0;
    LOG4CXX_INFO(logger,"PObservation "<<x<<"\n");
    LOG4CXX_INFO(logger,"  Discrete prior "<<s.p<<"\n");
    for(int i=0;i<s.p.n;i++) {
      if(s.p(i) != 0.0) {
	if(!s.x.empty()) {
	  Vector vtemp;
	  xRegressions[i].A.mul(s.x,vtemp);
	  LOG4CXX_ERROR(logger,"  Prediction "<<i<<": "<<vtemp+xRegressions[i].error.mu<<"\n");
	  ...
	  LOG4CXX_ERROR(logger,"  Prediction "<<i<<": "<<vtemp+xRegressions[i].error.mu<<"\n");
	  LOG4CXX_ERROR(logger,"  Error cov L "<<xRegressions[i].error.L<<"\n");
	  p += s.p(i) * xRegressions[i].Probability(s.xPrev,x);
	}
      }
    }
    LOG4CXX_INFO(logger,"  Probability of obs "<<p<<"\n");
    return p;
  }
}
...
void LinearProcessHMMRegression::Update(const LinearProcessHMMState& s,const Vector& x,LinearProcessHMMState& sobs) const
{
  ...
  for(size_t i=0;i<s.xy.size();i++) {
    ...
    for(size_t j=0;j<s.xy[i].means.size();j++) {
      ...
      /*
      LOG4CXX_INFO(logger,"Joint mean"<<s.xy[i].means[j]<<"\n");
      LOG4CXX_INFO(logger,"Joint cov"<<s.xy[i].covariances[j]<<"\n");
      LOG4CXX_INFO(logger,"Observation "<<x<<"\n");
      LOG4CXX_INFO(logger,"new mean: "<<sobs.y[i].means[j]<<"\n");
      LOG4CXX_INFO(logger,"new cov: "<<sobs.y[i].covariances[j]<<"\n");
      if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
      */
    ...
    if(sobs.p(i) > 1e-10 && !sobs.y[i].phi.empty()) {
      if(psum == 0.0) {
	LOG4CXX_WARN(logger,"LPHMMR warning: probability of discrete state "<<i<<": "<<sobs.p(i)<<", detailed obs prob sum "<<psum);
	//if(logger->isEnabledFor(log4cxx::Level::ERROR_INT)) getchar();
      }
      //Assert(psum != 0.0);
    }
